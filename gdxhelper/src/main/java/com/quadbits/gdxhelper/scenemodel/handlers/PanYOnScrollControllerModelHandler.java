...
import com.quadbits.gdxhelper.scenemodel.PanYOnScrollControllerModel;
 ...
 *
 */
public class PanYOnScrollControllerModelHandler extends ModelHandlerBaseImpl {
    @Inject
    ...
    @Inject
    public PanYOnScrollControllerModelHandler() {
        super();
    }
    ...
    public Object create(BaseModel model, String id, boolean forceCreation) {
        ...
        PanYOnScrollControllerModel controllerModel = (PanYOnScrollControllerModel) model;
        HashMap<String, Controller> allControllers = sceneModelManager.getAllControllers();

        // Create controller
        PanYOnScrollController controller = panYOnScrollControllerPool.obtain();
        ...
        // MinY and MaxY
        controller.setMinY(controllerModel.minY);
        controller.setMaxY(controllerModel.maxY);
    ...
    public int getPreferenceForModel(BaseModel model) {
        if (model.getClass() == PanYOnScrollControllerModel.class) {
            return HANDLER_PREFERENCE_FOR_MODEL_EXACT_MATCH;
        }

        if (model instanceof PanYOnScrollControllerModel) {
